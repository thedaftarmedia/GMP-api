...
export function parseSourceHtml(html, sourceUrl, now = new Date()) {
    ...
    $("article[itemtype*='FinancialProduct']").each((_, element) => {
        ...
        if (price === null && gmp === null && estimatedListing === null)
            return;
        records.push({
            name,
            category,
...
export async function fetchAndParseSource(sourceUrl = process.env.IPO_SOURCE_URL ?? DEFAULT_SOURCE_URL) {
    ...
    if (html.length < 5_000)
        throw new Error("IPO source returned unexpectedly small HTML");
