...
import { normalizeIpo, } from "../types/ipo.js";
...
iposRouter.get("/", async (_request, response) => {
    try {
        const documents = await callConvex("query", "ipos:listIpos", {});
        const groups = { mainboard: [], sme: [] };
        for (const document of documents) {
            const ipo = normalizeIpo(document);
            if (ipo.active)
                groups[ipo.category].push(ipo);
        }
        ...
        const payload = { success: true, available: true, data: groups };
...
iposRouter.get("/:id", async (request, response) => {
    try {
        const document = await callConvex("query", "ipos:getIpo", {
