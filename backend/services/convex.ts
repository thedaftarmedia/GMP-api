...
export async function callConvex<T>(
  ...
  try {
    const response = await fetch(
      `${convexUrl.replace(/\/$/, "")}/api/${functionType}`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new ConvexUnavailable(
        `Convex returned HTTP ${response.status}: ${errorBody}`,
      );
    }

    const body =
      (await response.json()) as ConvexResponse<T>;
    ...
    if (body.value === undefined) {
      throw new ConvexUnavailable(
        "Convex returned an invalid response",
      );
    }
    ...
    throw new ConvexUnavailable(
      "Convex is temporarily unavailable",
    );
  }
}